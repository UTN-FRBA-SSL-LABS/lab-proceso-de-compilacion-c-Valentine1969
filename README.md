...
> **R:Porque el preprocesador incluye headers, expande macros y elimina comentarios**
...
> **R:Porque el preprocesador elimina todos los comentarios del código fuente,
por lo tanto no aparecen en el archivo programa.i.**
...
> **R:Estas líneas indican el número de línea y el archivo original de donde proviene el código.
En este caso, muestran que el contenido proviene de stdio.h, que fue incluido por el preprocesador.
La función printf proviene de ese archivo.**
...
> **R:Estas instrucciones configuran la pila para la ejecución de la función
y luego acceden a la variable global "llamadas" para incrementarla en 1.**
...
> **R:Porque en programa.o la función area_circulo es utilizada pero no está definida,
ya que su implementación se encuentra en matematica.c.
En cambio, en matematica.o sí está definida, por eso aparece como tipo T.**
...
> **R:Sí, quedan símbolos de tipo U en el ejecutable final porque pertenecen a bibliotecas dinámicas del sistema.
Estos símbolos se resuelven en tiempo de ejecución por el sistema operativo.**
...
> **R:Una macro como CUADRADO(x) se reemplaza directamente en el código durante el preprocesamiento, antes de compilar, por lo que “desaparece” en esa etapa. No tiene verificación de tipos porque es solo una sustitución de texto.
En cambio, una función real como sumar(a, b) se compila normalmente y existe en el programa final. Tiene verificación de tipos durante la compilación, ya que el compilador controla que los argumentos sean correctos.**
...
> **R:Un símbolo de tipo T representa una función o código que está definido en la sección de texto (.text) del archivo objeto.
Un símbolo de tipo D representa una variable global con valor inicial, ubicada en la sección de datos (.data).
La diferencia es que T corresponde a código ejecutable, mientras que D corresponde a datos almacenados.**
